...
SELECT d.doctorID, d.doctorName, d.userID, u.userName, u.roles
FROM Users u
LEFT JOIN Doctors d ON d.userID = u.userID
WHERE u.roles = 'Doctor';
-- See the real userIDs of these doctor accounts:
SELECT userID, userName, email, phone FROM Users WHERE roles='Doctor';
INSERT INTO Doctors (doctorName, specialization, userID, email, phone, consultationFee)
VALUES ('Dr. Suksuk', 'General',18, 'suksuk@clinic.com', '0100000011', 50.00);

-- delete doctor users that have no Doctors profile (careful!)
DELETE FROM Users
WHERE roles='Doctor'
AND userID NOT IN (SELECT userID FROM Doctors WHERE userID IS NOT NULL);
...
-- =====================================================================
-- DATABASE CHANGES — run once in MySQL (skip any you already did)
-- =====================================================================

-- For prescriptions (multi-medicine, payment, dispense):
...
ALTER TABLE Prescriptions ADD COLUMN pharmacistID INT NULL;
...
-- For patient check-in:
ALTER TABLE Appointments ADD COLUMN checkedIn TINYINT(1) DEFAULT 0;
ALTER TABLE Appointments ADD COLUMN checkedInAt DATETIME NULL;

select * from PrescriptionItems;

-- NOTE: if a column already exists you'll get "Duplicate column name" -
-- that is harmless, just skip that line.
...
ALTER TABLE Prescriptions ADD COLUMN dispensed TINYINT(1) DEFAULT 0;
ALTER TABLE Prescriptions ADD COLUMN dispensedAt DATETIME NULL;
ALTER TABLE Prescriptions MODIFY medicineID INT NULL;
ALTER TABLE Prescriptions MODIFY quantity INT NULL;

ALTER TABLE Prescriptions DROP FOREIGN KEY prescriptions_ibfk_2;
ALTER TABLE Prescriptions DROP COLUMN medicineID;
